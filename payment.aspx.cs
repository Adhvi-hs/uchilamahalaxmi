...
namespace Temple
{
    public partial class payment : System.Web.UI.Page
    {
        ...
        {
            string expiration = TextBox2.Text;
            if (!IsValidExpiration(expiration))
            {
                Label8.Visible = true;
                Label8.Text = "Invalid expiration date.";
                return;
            }
            else
            {
                ...
                if (srd.Read())
                {
                    ...
                    if (status != "paid")
                    {
                        ...
                        string redirectScript = "setTimeout(function() { window.location.href = 'lhomepage.aspx'; }, 1000);"; // Delay of 3000 milliseconds (3 seconds)
